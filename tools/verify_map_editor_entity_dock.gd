...
func _run() -> void:
	...
	_expect(editor.tool_dock_container.get_child_count() == 1, "one terrain dock is shown")

	editor._show_entities_mode()
	_expect(editor._active_mode == &"entities", "entities mode activates")
	_expect(editor.tool_dock_container.get_child_count() == 1, "one entity dock is shown")
	...
	editor.entity_placement_controller.set_entity_settings({
	...
	editor.entity_placement_controller._ensure_entity_ghost()
	_expect(editor.entity_placement_controller.placement_ghost_root != null, "place mode creates a ghost preview")
	editor.entity_create_placement(editor.terrain.map_data.get_position_for_grid(Vector2i(128, 128)))
	...
	_expect(editor.entity_placement_controller.placement_preview_root.get_child_count() == before_count + 1, "placement preview appears")
	var preview_root: Node = editor.entity_placement_controller.placement_preview_root.get_child(editor.entity_placement_controller.placement_preview_root.get_child_count() - 1)
	...
	editor.entity_select_nearest_placement(placement.position)
	_expect(editor.entity_get_selected_placement_index() == before_count, "select mode selects existing placement")
	...
	editor.entity_move_selected_placement(moved_position)
	...
	editor.entity_delete_nearest_placement(placement.position)
